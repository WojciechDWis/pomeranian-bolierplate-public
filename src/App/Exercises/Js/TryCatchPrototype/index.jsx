export const Exercise = () => {
  ...
  function validate() {
    ...
    if (Number.isInteger(x) || isNaN(x)) throw 'Input to nie int';
  }
  ...
  const handleChange = (x) => {
    try {
      ...
      console.log('Validate - Wystąpił błąd', err);
    }
  };
  // -----------------------------------------------------------------------------
  // PROTOTYPE
  ...
  return (
    <div>
      <h1>TryCatchPrototype</h1>
      <div>
        <input id="numb" type="text" onChange={handleChange}></input>
        <div></div>
      </div>
    </div>
  );
